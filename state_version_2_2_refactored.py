# -*- coding: utf-8 -*-
"""State_Version_2.2_refactored.py

Refactored from State_Version_2.1.3 with three efficiency improvements:
...
Apply: set DEV_CACHE=1 in your environment to enable the dev cache.
"""
...
# --- 1. AUTHENTICATION ---
set_identity(os.getenv("EMAIL_EDGAR_API"))  # SEC EDGAR requires this

# --- 2. INITIALIZE MODELS ---
# Groq's max_retries handles rate limiting automatically (no time.sleep hack needed)
llm = ChatGroq(
...
# ============================================================
# FETCH HELPERS — each one is a standalone function so they can
# be parallelized via ThreadPoolExecutor and cached individually.
# ============================================================
...
# --- NODE: Financial Diagnostic (Chain-of-Thought) ---
...
# --- NODE: Sentiment (single batched FinBERT call for all three sources) ---
...
def sentiment_node(state: CompanyAnalysisState):
    ticker = state["ticker"]
    ...
    try:
        all_results = finbert(all_inputs, batch_size=16)
    except Exception as e:
